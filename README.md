...
```json
{ "title": "Rice", "description": "Miniket", "price": 70.0, "stockQty": 20 }
```

Response (201):

```json
{ "id": 10, "title": "Rice", "price": 70.0, "stockQty": 20, "active": true }
```
...
- Full-flow integration tests with `@SpringBootTest` + MockMvc:
  - application context + public pages load
  - buyer checkout flow (login → add to cart → checkout)
  - security and role restrictions
  - seller lifecycle (register pending → admin approval → seller product create)
  - product availability flow (sell out → blocked purchase → restock → purchase)
  - product lifecycle (buyer order history + admin status update to delivered)
